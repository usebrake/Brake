"""Phase 1: Unit tests for the LOCKITUP_TEST_MODE switch.
...
def _reset_env(value: str | None) -> None:
    if value is None:
        os.environ.pop("LOCKITUP_TEST_MODE", None)
    else:
        os.environ["LOCKITUP_TEST_MODE"] = value
...
def test_truthy_values() -> None:
    for v in ("1", "true", "TRUE", "yes", "on", "True"):
        _reset_env(v)
        from lockitup.test_mode import is_test_mode
...
def test_t_helper_returns_real_in_prod() -> None:
    _reset_env(None)
    from lockitup.test_mode import t
    assert t(600, 5) == 600
    assert t(1, 999) == 1
...
def test_shutdown_flag() -> None:
    ...
    from lockitup.test_mode import should_actually_shutdown as f2
    assert f2() is False
...
def test_watcher_constants_compress_under_test_mode() -> None:
    _reset_env("1")
    from lockitup.service import watcher as w_test
    assert w_test.PENALTY_MIN_SECONDS == 20

    _reset_env(None)
    from lockitup.service import watcher as w_prod
...
def test_probation_constant_compresses_under_test_mode() -> None:
    _reset_env("1")
    from lockitup import escalation as e_test
    assert e_test.PROBATION_SECONDS == 30

    _reset_env(None)
    from lockitup import escalation as e_prod
...
def main() -> int:
    ...
    saved = os.environ.get("LOCKITUP_TEST_MODE")
    try:
        for fn in tests:
            print(f"\n{fn.__name__}")
            fn()
