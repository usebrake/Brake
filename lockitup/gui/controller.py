...
Behavior:
  - If LockItUpService is running and responding on the IPC pipe, all writes
  ...
  - If the service is down (dev mode), falls back to direct StateStore access.
    Useful for `python -m lockitup.gui` without an install.
...
from lockitup.ipc.client import IPCClient, IPCError
