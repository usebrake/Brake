...
from lockitup.gui.assets import brake_icon, brake_icon_disabled
from lockitup.gui.controller import Controller


class LockItUpTray(QObject):
...
def install_tray(window: QMainWindow, controller: Controller) -> Optional[LockItUpTray]:
    """Convenience wrapper: only install if the system actually has a tray."""
    if not QSystemTrayIcon.isSystemTrayAvailable():
        return None
    return LockItUpTray(window, controller)
