...
def agent_command() -> list[str]:
    return exe_command("LockItUpAgent.exe", "lockitup.agent", windowless=True)


def lockout_command(args: list[str] | None = None) -> list[str]:
    return exe_command("LockItUpLockout.exe", "lockitup.lockout", args=args, windowless=True)


def service_command(args: list[str] | None = None) -> list[str]:
    return exe_command("LockItUpService.exe", "lockitup.service", args=args)


def watchdog_command(args: list[str] | None = None) -> list[str]:
    return exe_command("LockItUpWatchdog.exe", "lockitup.watchdog", args=args)
