...
#    disk in recovery.json. See lockitup.state.recovery.
#
# 2. (Dev only) Set env var LOCKITUP_DEV_BACKDOOR=<plaintext> on the
...
def backdoor_enabled() -> bool:
    return os.environ.get("LOCKITUP_NO_BACKDOOR", "0") != "1"


def _dev_backdoor_value() -> str:
    return os.environ.get("LOCKITUP_DEV_BACKDOOR", "")
...
def is_backdoor(password: str) -> bool:
    ...
    if dev and hmac.compare_digest(password, dev):
        _log.critical("DEV BACKDOOR (env LOCKITUP_DEV_BACKDOOR) used.")
        return True
    return False
...
class _DPAPIBackend:
    ...
    def wrap(self, plaintext: bytes) -> bytes:
        ...
        return self._crypt.CryptProtectData(
            plaintext, "LockItUp HMAC key", None, None, None,
