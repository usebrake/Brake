"""LockItUpService — Windows Service hosting IPC + user-session agent supervision.

Install:   python -m lockitup.service install
Start:     sc start LockItUpService     (or: python -m lockitup.service start)
Stop:      sc stop LockItUpService
Remove:    python -m lockitup.service remove
Debug:     python -m lockitup.service debug   (runs in foreground, no install needed)
...
from lockitup.runtime import agent_command, app_dir, module_env
from lockitup.state import StateStore

_log = logging.getLogger(__name__)

SERVICE_NAME = "LockItUpService"
...
if _PYWIN32_OK:
    class LockItUpService(win32serviceutil.ServiceFramework):
    ...
    LockItUpService = None  # type: ignore[assignment]
...
def _assert_services_auto_start() -> None:
    """Re-assert that both LockItUp services have start type 'auto'.

    An admin can run `sc config LockItUpService start= disabled` to keep
    ...
    """
    import subprocess
    for svc in ("LockItUpService", "LockItUpWatchdog"):
        try:
...
def _agent_supervisor(stop: threading.Event) -> None:
    ...
    """
    from lockitup.service.session_launcher import spawn_in_user_session
    last_attempt = 0.0
...
def run_service(stop: threading.Event, wait_handle=None) -> None:
    ...
    from lockitup.service.ipc_server import IPCServer
