...
Set the environment variable LOCKITUP_TEST_MODE=1 (in the same process that
...
sprinkle os.getenv("LOCKITUP_TEST_MODE") checks elsewhere — keep the switch
in one place so it's easy to audit and easy to remove for a release build.

Production behavior is the default: if LOCKITUP_TEST_MODE is unset/0/false,
...
def is_test_mode() -> bool:
    return os.environ.get("LOCKITUP_TEST_MODE", "").strip().lower() in _TRUTHY


def t(real_seconds: int, test_seconds: int) -> int:
    """Return test_seconds when LOCKITUP_TEST_MODE is on, else real_seconds."""
...
def log_banner_once() -> None:
    ...
    if is_test_mode():
        _log.warning(
            "LOCKITUP_TEST_MODE=1 — timers compressed and shutdown disabled. "
